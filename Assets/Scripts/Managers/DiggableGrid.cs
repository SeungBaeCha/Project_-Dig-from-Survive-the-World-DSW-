...
/// <summary>
/// 작은 '청크' 프리팹을 이용해 파낼 수 있는 지형을 격자 형태로 생성
/// 이 스크립트를 빈 게임 오브젝트에 추가하고, Inspector에서 격자 크기와 청크 프리팹 설정
/// </summary>
public class DiggableGrid : MonoBehaviour
{
    [Header("Grid Dimensions")]
    [Tooltip("격자의 너비 (X-axis) 가로측")]
    public int width = 10;
    [Tooltip("격자의 높이 (Y-axis) 세로측")]
    public int height = 5;
    [Tooltip("격자의 깊이 (Z-axis) 안쪽")]
    public int depth = 10;
    ...
    [Tooltip("각 청크의 크기. 프리팹의 스케일과 일치해야함")]
    ...
    void Start()
    {
        ...
        if (chunkPrefab == null)
        {
            Debug.LogError("Chunk Prefab이 할당되지 않았습니다! DiggableGrid 스크립트의 Inspector에서 프리팹을 설정해주세요.");
            return;
        }
